 ...
 */
export async function generarPDFPreview(factura: any, items: any[], isPremium: boolean, plantilla: PlantillaPDF = 'default', simboloMoneda: string = '€'): Promise<string | null> {
  try {
    const html = await generarHTMLFactura(factura, items, isPremium, plantilla, simboloMoneda);
    const { uri } = await Print.printToFileAsync({ html, base64: false });
    return uri;
  } catch (error) {
 ...
 */
async function generarHTMLFactura(factura: any, items: any[], isPremium: boolean, plantilla: PlantillaPDF = 'default', simboloMoneda: string = '€'): Promise<string> {
  const fechaCreacion = new Date(factura.fecha).toLocaleDateString('es-ES', {
    day: '2-digit', month: 'long', year: 'numeric'
  });
